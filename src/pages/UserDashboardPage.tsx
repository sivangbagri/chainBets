...
export const UserDashboardPage: React.FC<{ addresses: ContractAddresses }> = ({
  addresses,
}) => {
  ...
  const handleClaim = async (matchId: number) => {
    ...
    try {
      ...
      const setTx = await contracts.bettingPool.setCommunityHub(
        addresses.communityHub
      );
      await setTx.wait();
  ...
  const getBetStatus = (bet: (typeof bets)[0]) => {
    ...
    if (bet.winner === bet.prediction)
      return { label: "won", color: "bg-green-200" };
    return { label: "lost", color: "bg-red-200" };
  };

  if (serviceLoading || betsLoading)
    return (
      <>
        <Loader />
      </>
      ...
      <div className="bg-[#0B0F13] border border-[#1C2127] rounded-xl overflow-hidden">
        <table className="min-w-full divide-y divide-[#1C2127]">
          <thead className="bg-[#0F1418]">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-medium text-[#A1A1A6]">
                MATCH
              </th>
              <th className="px-6 py-4 text-left text-sm font-medium text-[#A1A1A6]">
                BET AMOUNT
          ...
          <tbody className="divide-y divide-[#1C2127]">
            {bets.map((bet) => {
              ...
              return (
                <tr
                  key={bet.matchId}
                  className="hover:bg-[#0F1418] transition-colors"
                >
                  ...
                  <td className="px-6 py-4 text-sm">
                    ...
                    ${
                      status.color === "bg-green-200"
                        ? "bg-[#1C2127] text-green-500"
                        : status.color === "bg-yellow-200"
                        ? "bg-[#1C2127] text-[#FFB800]"
